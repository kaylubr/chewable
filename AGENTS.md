...
## Project Overview
A privacy-conscious photobooth web app. Guests use the full experience (frame selection, webcam capture, Canvas composition, download) with no account. Authentication is only required to permanently save a finished photo.
...
- Backend layout: `app/{main.py, core/{config.py, security.py}, api/{router.py, auth.py, photos.py}, models/, schemas/, services/, db/, repositories/, tests/}`.
